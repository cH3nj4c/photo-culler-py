...
pc_ui.filedialog.askdirectory = lambda *args, **kwargs: str(photo_dir)
pc_ui.messagebox.showinfo = lambda *args, **kwargs: None
pc_ui.messagebox.showwarning = lambda *args, **kwargs: None
pc_ui.messagebox.showerror = lambda *args, **kwargs: None
pc_ui.messagebox.askyesno = lambda *args, **kwargs: True
...
def probe():
    try:
        ...
        # Preview recovery is async after delete; give the loader a moment.
        app.after(500, verify_after_delete)


def verify_after_delete():
    try:
        result["preview_ok"] = (
            app.preview_photo is not None and app.current_source_path is not None
        )
...
pc_ui.filedialog.askdirectory = lambda *args, **kwargs: str(pair_dir)
...
def probe_pair():
    try:
        ...
        pair_app.after(500, verify_pair_after_delete)


def verify_pair_after_delete():
    try:
...
pair_app.after(2200, probe_pair)
pair_app.mainloop()
